...
let log2 = function (number) {
    return Math.log(number) / Math.log(2);
};

let buildProgramWrapper = function (gl, vertexShader, fragmentShader, attributeLocations) {
    let programWrapper = {};
...
let buildTexture = function (gl, unit, format, type, width, height, data, wrapS, wrapT, minFilter, magFilter) {
...
let buildFramebuffer = function (gl, attachment) {
...
let OCEAN_FRAGMENT_SOURCE = [
    ...
    'void main (void) {',
        ...
        'vec3 cameraReflectDir = vec3(u_viewMatrix * vec4(reflectDir, 0.0));',
        'float angle = max(dot(view, cameraReflectDir), 0.0);',
...
let Simulator = function () {
    ...
    let fullscreenVertexShader = buildShader(gl, gl.VERTEX_SHADER, FULLSCREEN_VERTEX_SOURCE);

    let horizontalSubtransformProgram = buildProgramWrapper(gl, fullscreenVertexShader, 
    ...
    let verticalSubtransformProgram = buildProgramWrapper(gl, fullscreenVertexShader, 
    ...
    let initialSpectrumProgram = buildProgramWrapper(gl, fullscreenVertexShader, 
    ...
    let phaseProgram = buildProgramWrapper(gl, fullscreenVertexShader, 
    ...
    let spectrumProgram = buildProgramWrapper(gl, fullscreenVertexShader, 
    ...
    let normalMapProgram = buildProgramWrapper(gl, fullscreenVertexShader, 
    ...
    let oceanProgram = buildProgramWrapper(gl,
    ...
    let fullscreenVertexBuffer = gl.createBuffer();
    ...
    let oceanBuffer = gl.createBuffer();
    ...
    let oceanIndexBuffer = gl.createBuffer();
    ...
    let initialSpectrumTexture = buildTexture(gl, INITIAL_SPECTRUM_UNIT, gl.RGBA, gl.FLOAT, RESOLUTION, RESOLUTION, null, gl.REPEAT, gl.REPEAT, gl.NEAREST, gl.NEAREST),
    ...
    let pingPhaseTexture = buildTexture(gl, PING_PHASE_UNIT, gl.RGBA, gl.FLOAT, RESOLUTION, RESOLUTION, phaseArray, gl.CLAMP_TO_EDGE, gl.CLAMP_TO_EDGE, gl.NEAREST, gl.NEAREST);

    let initialSpectrumFramebuffer = buildFramebuffer(gl, initialSpectrumTexture),
