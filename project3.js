...
function draw() {
    ...
    let inverseTranspose = new Matrix4(g_i400Matrix).invert().transpose();
    gl.uniformMatrix4fv(g_u_inversetranspose_ref, false, inverseTranspose.elements);
    gl.uniform3fv(g_u_light_ref, new Float32Array([0, 100, 0]))
...
function generateIsland(size) {
    ...
    VBO2 = initVBO(new Float32Array(g_islandMesh.concat(islandColors).concat(g_islandMesh).concat(g_islandMesh)));
}
...
let g_u_inversetranspose_ref, g_u_light_ref, g_u_specpower_ref, g_u_texture_ref;
let g_u_flatlighting_ref;
let g_meshLen;
function startRendering() {
    ...
    const data = g_i400BodyMesh.concat(g_i400HatchMesh).concat(g_i400PropMesh).concat(g_planeMesh).concat(g_uboatMesh).concat(g_torpMesh).concat(g_explMesh).concat(g_seaMesh)
        ...
        .concat(g_i400BodyNormals).concat(g_i400HatchMesh).concat(g_i400PropMesh).concat(g_planeMesh).concat(g_uboatMesh).concat(g_torpMesh).concat(g_explMesh).concat(g_seaMesh)
        .concat(g_i400BodyTexCoords).concat(g_i400HatchMesh).concat(g_i400PropMesh).concat(g_planeMesh).concat(g_uboatMesh).concat(g_torpMesh).concat(g_explMesh).concat(g_seaMesh);
    ...
    let texturePointer = gl.createTexture()
    gl.bindTexture(gl.TEXTURE_2D, texturePointer)
    gl.uniform1i(g_u_texture_ref, texturePointer)
    ...
    g_meshLen = g_i400BodyMesh.length + g_i400HatchMesh.length + g_i400PropMesh.length + g_planeMesh.length
        + g_uboatMesh.length + g_torpMesh.length + g_explMesh.length + g_seaMesh.length;
}
...
let g_torpMesh = [];
let g_explMesh = [];
let g_i400BodyNormals = [];
let g_i400BodyTexCoords = [];
async function loadOBJFiles() {
    const files = [
        { path: './resources/I400Body2.obj', mesh: g_i400BodyMesh, normals: g_i400BodyNormals, texCoords: g_i400BodyTexCoords },
    ...
    await Promise.all(files.map(async (file) => {
        ...
        if (file.normals) {
            return readObjFile(data, file.mesh, file.normals, file.texCoords);
        }
